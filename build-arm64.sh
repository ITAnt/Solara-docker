...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Solara Music ARM64 镜像构建脚本${NC}"
echo -e "${BLUE}========================================${NC}"
...
# 检查是否存在旧镜像
if docker images | grep -q "${IMAGE_NAME}:${VERSION}-arm64"; then
    echo -e "${YELLOW}⚠ 发现旧的 ARM64 镜像${NC}"
    echo -e "${BLUE}删除旧镜像...${NC}"
    docker rmi -f ${IMAGE_NAME}:${VERSION}-arm64 2>/dev/null || true
...
# 导出 ARM64 镜像
OUTPUT_FILE="${OUTPUT_DIR}/${IMAGE_NAME}-${VERSION}-arm64.tar"
...
docker save ${IMAGE_NAME}:${VERSION}-arm64 -o ${OUTPUT_FILE}
...
echo ""
echo -e "${BLUE}运行容器:${NC}"
echo -e "  docker run -d -p 3001:3001 -v ./downloads:/download --name solara-music ${IMAGE_NAME}:${VERSION}-arm64"
echo ""
